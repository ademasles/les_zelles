# GitHub Copilot Prompt - T8.2 Add RAG Regression Dataset
...
Implement ticket T8.2: Add RAG Regression Dataset.

## Objective

Create a small evaluation dataset for retrieval/citation behaviour without requiring a live LLM in CI.
...
1. Create rag_cases.json with 3-5 cases.
2. Test retrieval returns expected evidence.
3. Test answers include citations with mocked LLM.
4. Add no-evidence case.
5. Mark live LLM tests separately if any.
...
- [ ] RAG regression tests exist.
- [ ] CI does not require Ollama.
- [ ] Citations/evidence behaviour is tested.
```
