# GitHub Copilot Prompt - T9.1 Improve Backend Dockerfile
...
Implement ticket T9.1: Improve Backend Dockerfile.

## Objective

Improve backend Dockerfile for pyproject install, system dependencies, and local-first runtime.
...
2. Install Tesseract and language packs; LibreOffice only if needed.
3. Install backend using `pip install -e .`.
4. Do not copy `.env`.
5. Use uvicorn app.main:app command.
6. Consider non-root user if safe.
...
- [ ] System OCR dependencies are present.
